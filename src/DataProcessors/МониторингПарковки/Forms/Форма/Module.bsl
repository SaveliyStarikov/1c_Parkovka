	...
	Для каждого СтрокаПлощадки Из ТабПлощадки Цикл
		...
		Для каждого СтрокаРяд Из ТабРяды Цикл 
			...
			Для каждого СтрокаМеста Из МассивМест Цикл
				...
				ПолеМеста = Элементы.Добавить(ИмяГруппыМеста, Тип("ПолеФормы"),ГруппаРяд);
				ПолеМеста.Вид = ВидПоляФормы.ПолеВвода;
				//ПолеМеста.Вид = 
				...
				КомандаМесто.Заголовок = ?(СтрокаМеста.Состояние,"Забронировать","Освободить");
...
Функция ОбновитьЭлементыМеста(СтрокаМеста)
	...
	//ГруппаКнопка = Элементы.Найти(ИмяКнопки);
	
	Если СтрокаМеста.Состояние Тогда
		ГруппаМесто.ЦветФона = Новый Цвет(0, 255, 127); // Зеленый - свободно
		ТекстАвто = "Состояние";
		//ГруппаКнопка.Заголовок = "Занять";
	Иначе
		ГруппаМесто.ЦветФона = Новый Цвет(255, 128, 114); // Красный - занято
		Если СтрокаМеста.ТранспортноеСредство = Неопределено Или СтрокаМеста.ТранспортноеСредство.Пустая() Тогда
			ТекстАвто = "Машина";
