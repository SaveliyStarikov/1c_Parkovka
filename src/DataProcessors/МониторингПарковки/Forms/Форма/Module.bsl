...
Функция ВыборкаМест()
	...
	|	Ряды.Ссылка КАК Ряд,
	|	Ряды.Наименование КАК НаименованиеРяды,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Ряды КАК Ряды
	|		ПО Площадка1.Ссылка = Ряды.Владелец
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Места КАК Места
	|		ПО (Ряды.Ссылка = Места.Владелец)
	|ГДЕ
	|	НЕ Ряды.ПометкаУдаления
...
Процедура ОбновитьЭлементыМест()
    ...
    // Удаляем только визуальные элементы, команды НЕ трогаем
    Если Элементы.Найти("ГруппаСетка") <> Неопределено Тогда
        Элементы.Удалить(Элементы.ГруппаСетка);
    КонецЕсли;
    ...
    Пока ВыборкаПлощадки.Следующий() Цикл
        ...
        Пока ВыборкаРяды.Следующий() Цикл
            ...
            ИмяРяда = СтрЗаменитьПоРегулярномуВыражению(ВыборкаРяды.НаименованиеРяды,"[^a-zA-Z0-9а-яА-ЯёЁ]","_");
            ...
            Пока ВыборкаМеста.Следующий() Цикл
                ...
                ИмяКоманды = "КомандаМесто_" + Индекс;
                КомандаМесто = Команды.Найти(ИмяКоманды);
                
                Если КомандаМесто = Неопределено Тогда
                    // Команды нет - создаем новую
                    КомандаМесто = Команды.Добавить(ИмяКоманды);
                    КомандаМесто.Действие = "ОткрытьФормуМеста";
                КонецЕсли;
                
                // Обновляем заголовок команды (меняется в зависимости от статуса "Свободно/Занято")
